...
if [ -z $outdir ]; then outdir=$site; fi
...
if [ ! -d $outdir/data_ap+anaticor/$s ]; then mkdir -p $outdir/data_ap+anaticor/$s; fi

# before starting we have to generate de WM mask from the animal warper files

basedir=$PWD

cd ${data_aw}/${s}

# calculate the WM_mask

singularity exec  --cleanenv $container 3dcalc -overwrite -a SEG_in_${s}_anat.nii.gz \
 -expr "amongst(a,4)" -prefix WM_in_${s}_anat.nii.gz
...
if [ $multruns -eq 0 ]; then
	...
	singularity exec -B /misc:/misc --cleanenv $container afni_proc.py \
		-subj_id $s \
		-script $outdir/data_ap+anaticor/$s/proc.$s -scr_overwrite \
		...
		-mask_import Tvent ${data_aw}/${s}/ VENT_in_${s}_anat.nii.gz \
		-mask_import WMe ${data_aw}/${s}/WM_in_${s}_anat.nii.gz \
		-mask_
	...
	singularity exec -B /misc:/misc --cleanenv $container 3dAFNItoNIFTI \
		-prefix $outdir/data_ap+anaticor/$s/$s.results/errts.$s.anaticor.tproject+tlrc.nii.gz \
		$outdir/data_ap+anaticor/$s/$s.results/errts.$s.tproject+tlrc.

		# convert mask to NIFTI
		masks=$(find $outdir/data_ap+anaticor/$s/$s.results -type f -name "*mask*.BRIK" )

		for mask in $masks
		do
		singularity exec -B /misc:/misc --cleanenv $container 3dAFNItoNIFTI $mask
		done
	gzip$outdir/data_ap+anaticor/$s/$s.results/*.nii

	echo "Bringing down BRIKs and chopping HEADs..."
  rm $outdir/data_ap+anaticor/$s/$s.results/*.BRIK $outdir/data_ap+anaticor/$s/$s.results/*.HEAD
	...
	for epi in ${s_epi[@]}; do
		...
		singularity exec -B /misc:/misc --cleanenv $container afni_proc.py \
			-subj_id $s \
			-script $outdir/data_ap+anaticor/$s/proc.${s}_${ses}_${run} -scr_overwrite \
			-out_dir $outdir/data_ap+anaticor/$s/${s}_${ses}_${run}.results \
			-blocks despike tshift align tlrc volreg blur mask scale regress \
			-dsets $epi \
			-copy_anat "${s_anat}" \
			-anat_has_skull no \
		...
		echo "Converting errts.$s.tproject+tlrc to NIFTI because who uses BRIK?"
		singularity exec -B /misc:/misc --cleanenv $container 3dAFNItoNIFTI \
			-prefix $outdir/data_ap+anaticor/$s/${s}_${ses}_${run}.results/errts.$s.anaticor.tproject+tlrc.nii.gz \
			$outdir/data_ap+anaticor/$s/${s}_${ses}_${run}.results/errts.$s.tproject+tlrc.


		# convert mask to NIFTI
		masks=$(find $outdir/data_ap+anaticor/$s/${s}_${ses}_${run}.results -type f -name "*mask*.BRIK" )

    for mask in $masks
		do
		singularity exec -B /misc:/misc --cleanenv $container 3dAFNItoNIFTI $mask
		done
gzip $outdir/data_ap+anaticor/$s/${s}_${ses}_${run}.results/*.nii
	...
	echo "Bringing down BRIKs and chopping HEADs..."
	rm $outdir/data_ap+anaticor/$s/${s}_${ses}_${run}.results/*.BRIK $outdir/data_ap+anaticor/$s/${s}_${ses}_${run}.results/*.HEAD
