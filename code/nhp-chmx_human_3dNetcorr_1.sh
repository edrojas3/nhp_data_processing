...
data_ap=$site/data_ap/anaticor/$subj/$subj.results
...
3dcopy $data_ap/anatQQ.${subj}+tlrc $outdir/${subj}_anat_warp2std_nsu

## epi in standard space
cp $data_ap/errts.${subj}.fanaticor+tlrc.nii.gz $outdir/errts.${subj}.fanaticor+tlrc.nii.gz
...
# Resample ROIs
3dresample -master $outdir/errts.${subj}.fanaticor+tlrc.nii.gz \
...
3dcalc -a $outdir/${subj}_anat_warp2std_brainmask2epi+tlrc \
		-b $outdir/errts.${subj}.fanaticor+tlrc.nii.gz             \
		-expr 'a*b'                                        \
		-prefix $outdir/errts.${subj}.fanaticor.masked

# Extract an example volume from EPI for visualization purposes
3dTcat $outdir/errts.${subj}.fanaticor.masked+tlrc'[10]'    \
		-prefix $outdir/errts.${subj}.fanaticor.examp
...
3dNetCorr -echo_edu                                         \
		...
		-fish_z                                             \
		-inset $outdir/errts.${subj}.fanaticor.masked+tlrc   \
		-in_rois $rois                                       \
		-ts_out												\
		-ts_wb_corr                                         \
		-ts_wb_Z                                            \
