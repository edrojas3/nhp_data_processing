...
# slice based regressors for cardio and respiration

ricor_files=$(find $site/${s} -type f -name "*.slibase.1D")
...
for epi in ${s_epi[@]}; do
		...
		ricor_file=$(echo $ricor_files |sed 's/ /\n/g' | grep $run)
